...
// Multiple Regression using real_estate.csv dataset (Batch gradient descent)
void example_linear_regression_bgd(){
...
// Multiple Regression using real_estate.csv dataset (Stochastic gradient descent)
void example_linear_regression_sgd(){
	std::string url = ".\\dataset\\real_estate.csv";
	Dataset dataset(url, 80);
	MatrixXd train = dataset.get_train();
	MatrixXd test = dataset.get_test();
	...
	double iterations = 100000;
	Linear linear(x_train, y_train, weights, bias);
	MatrixXd y_predict = linear.train(x_test, y_test, learning_rate, "sgd", epsilon, iterations, 1000);
